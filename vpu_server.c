...
int main(int argc, char* argv[])
{
	...
	int yuv_fd1, yuv_fd2;
	int num = 1000;
	vs_context* client_context;
	vs_frame* frame_client;
	vs_frame* frame_server;
	vs_context* server_context;
	struct camera_buf* camera_buf;
	int raw_image_size = VS_FRAME_WIDTH*VS_FRAME_HEIGHT*3/2;

	yuv_fd1 = open("sample1.yuv", O_RDWR);
	yuv_fd2 = open("sample2.yuv", O_RDWR);
	nand_open();
#ifdef CAMERA
	camera_open();
	camera_init_channel(capture_channel, VS_FRAME_WIDTH, VS_FRAME_HEIGHT);
#endif
	server_context = vs_server_create_context();
	//printf("%s: 1\n", __func__);
	client_context = vs_client_create_context();
	//printf("%s: 2\n", __func__);

	{
		unsigned char* buf;
		int size;
		vs_client_get_header(client_context, 0, &buf, &size);
		nand_write(buf, size);
	}
	...
	while(num--){
		int i;
		frame_client= vs_client_get_idle_frame(client_context, 0);
#ifdef CAMERA
		camera_start_channel(capture_channel);
		camera_buf = camera_get_data(capture_channel);
		
		yuv420_deinterlace(
				camera_buf->addr,
				frame_client->desc.virt_uaddr,
				VS_FRAME_WIDTH,
				VS_FRAME_HEIGHT);
#endif

		if(num % 2){
			ret = read(yuv_fd1, frame_client->virt_client, raw_image_size);
		}else{
			ret = read(yuv_fd2, frame_client->virt_client, raw_image_size);
		}
	
		vs_client_queue_frame(client_context, frame_client);
#ifdef CAMERA
		camera_finish_data(capture_channel, camera_buf);
#endif

		vs_server_get_filled_frame(server_context, &frame_server);
		vs_server_encode_frame(server_context, frame_server);

		{
			unsigned char* buf;
			int size;
			vs_client_get_output_info(client_context, frame_client, &buf, &size);
			nand_write(buf, size);
			vs_client_consum_encoded_data(client_context, frame_client, size);
		}
	}

#ifdef CAMERA
	camera_stop_channel(capture_channel);
	camera_destroy_channel(capture_channel);
	camera_close();
#endif
	nand_flush();
	nand_close();
	vs_client_destroy_context(client_context);
	vs_server_destroy_context(server_context);
}
#endif
