...
#define IMAGE_LIB_VERSION "ImageLib for android V0.1.0(from v2.11.10)"
...
typedef void *         		 IMG_T_pVOID;     /* pointer of void data */
...
typedef IMG_T_BOOL (*CALLBACK_FUN_REGMODIFY)(IMG_T_U32 addr, IMG_T_U32 value, IMG_T_U32 mask); // 修改系统寄存器


typedef IMG_T_pVOID (*IMGLIB_CALLBACK_FUN_DMA_MALLOC)(IMG_T_U32 size);// 分配物理内存  
typedef IMG_T_VOID (*IMGLIB_CALLBACK_FUN_DMA_FREE)(IMG_T_pVOID mem); // 释放物理内存 
typedef IMG_T_pVOID (*IMGLIB_CALLBACK_FUN_VADDR_TO_PADDR)(IMG_T_pVOID mem);// 物理内存虚拟地址转物理地址


typedef IMG_T_U32 (*IMGLIB_CALLBACK_FUN_MAP_ADDR)(IMG_T_U32 phyAddr, IMG_T_U32 size);// 映射寄存器组（仅适用于jpg私有寄存器组）
typedef IMG_T_VOID (*IMGLIB_CALLBACK_FUN_UNMAP_ADDR)(IMG_T_U32 addr, IMG_T_U32 size);  // 取消寄存器组映射

/* 硬件使用的互斥，blocked为1表示一直阻塞直到硬件资源可用，为0表示不可用直接返回*/
/*更新:去掉第二个参数*/
typedef IMG_T_pVOID (*IMGLIB_CALLBACK_FUN_VIDEO_HW_LOCK)(IMG_T_S32 hw_id, IMG_T_S32 blocked);
typedef IMG_T_S32 (*IMGLIB_CALLBACK_FUN_VIDEO_HW_UNLOCK)(IMG_T_pVOID hLock);
...
typedef struct{
	...
	// add by ycx
    IMGLIB_CALLBACK_FUN_DMA_MALLOC			dma_malloc;
    IMGLIB_CALLBACK_FUN_DMA_FREE			dma_free;
    IMGLIB_CALLBACK_FUN_VADDR_TO_PADDR		vaddr2paddr;
    IMGLIB_CALLBACK_FUN_MAP_ADDR			reg_map;
    IMGLIB_CALLBACK_FUN_UNMAP_ADDR			reg_unmap;
    IMGLIB_CALLBACK_FUN_VIDEO_HW_LOCK		hw_lock;
    IMGLIB_CALLBACK_FUN_VIDEO_HW_UNLOCK		hw_unlock;
    
} CB_FUNS;
#endif

typedef IMG_T_VOID (*FlushCacheFunc)();

typedef IMG_T_VOID (*SaveBuffFunc)(IMG_T_U8 *buff, IMG_T_U32 size);

typedef IMG_T_VOID (*JPEGTaskFunc)();
...
typedef struct
{
	...
	IMG_T_U8  alpha;        // OSD和背景色混合度，取值范围[0,16]。0：不透明；16：全透明
...
* @RETURN 图像库版本信息字符串
*/
IMG_T_S8 *Img_GetVersionInfo();

/** 获得最后一次出错的出错代码
* @RETURN 出错代码的枚举值(参见本文件中的T_IMAGE_ERROR枚举类型定义)
*/
T_IMAGE_ERROR Img_GetLastError();
